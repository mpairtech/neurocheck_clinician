...
const AssessmentDetails = () => {
  ...
  // Derive early so the useEffect below can reference them
  ...
  /* ---------------- REPORT DATA SHAPE ---------------- */
  // ✅ Always called — no early return above this
  useEffect(() => {
    if (!data) return;
    setReportData({
  ...
  /* ---------------- PERSIST SECTIONS ---------------- */
  // Called by AssessmentReport whenever a section heading is edited or a new
  // section is added. Decides between create vs update based on whether an
  // appointment record already exists for this patient.
  const handleSectionsChange = async (updatedSections) => {
    // 1. Optimistic update — UI reflects the change immediately
    setReportData((prev) => ({ ...prev, feedbackSections: updatedSections }));

    setIsSavingSections(true);
    try {
      const feedbackPayload = {
        feedback: { sections: updatedSections },
      };

      if (patientAppointment?.id) {
        // Appointment exists → update it
        await updateAppointment(patientAppointment.id, feedbackPayload);
      } else {
        // No appointment yet → create one on the fly
        await addAppointment({
          patientId: submissionPatientId,
          clinicianId: Number(userData?.id),
          ...feedbackPayload,
        });
      }

      // Refresh so patientAppointment.id is available for subsequent saves
  ...
  /* ---------------- OTHER HANDLERS ---------------- */
  const handleDownloadReport = () => reportRef.current?.downloadPDF();
  ...
  const handleSubmitFeedback = async (e) => {
    ...
    await updateSchedule(selectedAppointment.id, { feedback: notes });
    closeFeedBackModal();
    fetchAppointments();
  };
  ...
  const handleReportFormSubmit = async (formattedNotes) => {
    if (!selectedAppointment) return;
    await updateSchedule(selectedAppointment.id, {
      feedback: { sections: formattedNotes.sections },
    });
    setShowReportForm(false);
    fetchAppointments();
  };

  const handleMakeDiagnosis = () => navigate(`/prescription/${patientId}`);
  ...
  /* ---------------- DERIVED VALUES (data guaranteed here) ---------------- */
