...
const S = {
  ...
  // ── A4 standard side margins (25 mm ≈ 71 px) ──────────────────────────
  inner: { padding: "48px 70px 100px" },
  pageHeader: {
    display: "flex",
...
// ── Section heading: line above + badge + title + line below ─────────────────
...
// Strip tags for height estimation only
function quillToPlainForHeight(html = "") {
  if (!html) return "";
  return html
    .replace(/<[^>]+>/g, " ")
    .replace(/&nbsp;/g, " ")
...
// Convert a DOM node to a React element with inline styles (html2canvas-safe)
function nodeToReact(node, key, listType, listIndex) {
  if (node.nodeType === 3) {
    // TEXT_NODE
    return node.textContent;
  }
  const tag = node.tagName?.toLowerCase();
  // children computed here for non-list tags; list tags compute their own
  const children = Array.from(node.childNodes).map((c, i) => nodeToReact(c, i));
  ...
  switch (tag) {
    case "p": {
      ...
      return (
        <p
          key={key}
          style={{
            margin: empty ? "0" : "0 0 8px 0",
            height: empty ? "8px" : "auto",
            lineHeight: "1.7",
            textAlign: "justify",
            fontFamily: "'Georgia', serif",
            fontSize: "13px",
    ...
    case "ul":
    case "ol": {
      // Quill v2 uses <ol> for both bullets and numbered lists,
      // differentiated by data-list attr on each <li>
      let orderedIndex = 0;
      return (
        <div key={key} style={{ margin: "4px 0 8px 0" }}>
          {Array.from(node.childNodes).map((c, i) => {
            const dataList = c.getAttribute?.("data-list") || "";
            const isOrdered = tag === "ol" && dataList !== "bullet";
    ...
    case "li": {
      // listType / index are passed from the parent ul/ol handler above
      ...
      // Filter out Quill's ql-ui spans (they're decorative only)
      const liChildren = Array.from(node.childNodes)
        .filter((c) => !(c.nodeType === 1 && c.classList?.contains("ql-ui")))
        .map((c, i) => nodeToReact(c, i));
      return (
        <div
          key={key}
          style={{
      ...
      return (
        <div
          ...
          style={{
            ...
            fontWeight: "bold",
            color: T.dark,
            fontSize: tag === "h1" ? "18px" : tag === "h2" ? "16px" : "14px",
            margin: "12px 0 6px",
          }}
        >
          {children}
...
// Parse Quill HTML and render as inline-styled React elements
function renderQuillHTML(html) {
  if (!html) return null;
  const doc = new DOMParser().parseFromString(html, "text/html");
  return (
    <div>
      {Array.from(doc.body.childNodes).map((node, i) => nodeToReact(node, i))}
    </div>
  );
}
...
function makeBlocks(data, submission, today, feedbackSections = []) {
  ...
  feedbackSections.forEach((section) => {
    const plainText = quillToPlainForHeight(section.content);
    const lineCount = plainText
      .split(" ")
      .reduce((acc, _, i, arr) => (i % 92 === 0 ? acc + 1 : acc), 1);
    ...
    add(
      <div key={`section-${sectionNum}`} >
        ...
        />
        <div>{renderQuillHTML(section.content)}</div>
      </div>,
      totalH,
    );
    sectionNum++;
